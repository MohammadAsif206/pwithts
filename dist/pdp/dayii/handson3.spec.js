...
test_1.test.describe("Handle table", () => {
    test_1.test.beforeEach("Navigate to the testing webpage", async ({ page }) => {
        await page.goto('https://playground.bondaracademy.com/pages/tables/smart-table');
        await (0, test_1.expect)(page).toHaveURL(/smart-table/);
    });
    (0, test_1.test)("Filter Specific Field and Validate Result", async ({ page }) => {
        ...
        const emailFilter = page.locator('input[placeholder="E-mail"]');
        // Enter filter value
        await emailFilter.fill(emailId);
        await page.keyboard.press('Tab');
        const rows = page.locator('tbody tr');
        // Verify at least one row exists
        await test_1.expect.poll(async () => rows.count()).toBeLessThan(5);
        ...
        for (let i = 0; i < rowCount; i++) {
            ...
            (0, test_1.expect)(actualValue?.trim()).toBe(emailId);
        }
    });
    (0, test_1.test)("Select 20 Entries and Delete Second Row", async ({ page }) => {
        ...
        page.once('dialog', async (dialog) => {
            (0, test_1.expect)(dialog.message()).toContain('Are you sure');
            await dialog.accept();
        });
        await secondRowDelete.click();
        const firstPage1 = page.locator('table tbody tr');
        await test_1.expect.poll(async () => await firstPage1.count()).toEqual(10);
        await page.getByRole('link', { name: '2' }).click();
        const secondPage1 = page.locator('table tbody tr');
        await test_1.expect.poll(async () => await secondPage1.count()).toEqual(10);
        ...
        (0, test_1.expect)(secondRowBeforDel).not.toEqual(secondRowAfterDel);
    });
    (0, test_1.test)("Handle Date Picker Using Dynamic Function", async ({ page }) => {
        await page.goto("https://demoqa.com/date-picker");
        await (0, test_1.expect)(page).toHaveURL(/date-picker/);
        ...
        (0, test_1.expect)(dateSelected).toEqual(expected);
    });
    (0, test_1.test)('Handle Nested Iframe', async ({ page }) => {
        ...
        (0, test_1.expect)(confirmation).toContain('Registration Form is Successfully Submitted');
        ...
        (0, test_1.expect)(message).toEqual('You clicked on OK!');
        await popup.getByText('Confirm Alert Box', { exact: true }).click();
        const message1 = await popup.locator('#demo').textContent();
        (0, test_1.expect)(message1).toEqual('You clicked on Cancel!');
        await popup.getByText('Prompt Alert Box', { exact: true }).click();
        const message2 = await popup.locator('#demoone').textContent();
        (0, test_1.expect)(message2).toEqual('User cancelled the prompt.');
    });
    // No part of hands 4- more practice material
    test_1.test.only("Handle nested ifram - child frame Project Detaisl", async ({ page }) => {
        await page.goto("https://vinothqaacademy.com/iframe/");
        (0, test_1.expect)(page).toHaveURL(/ifram/);
        await page.locator(".iframe-container").click();
        const iFrame = page.frameLocator("iframe[name='employeetable']");
        (0, test_1.expect)(await iFrame.getByText("Project Details").textContent()).toEqual("Project Details");
        //add an employee
        ...
        const expectedVals = lastRow.split("\t");
        console.log("First ", expectedVals[0]);
        test_1.expect.soft(expectedVals[0].trimStart() + "" + expectedVals[1].trim()).toEqual(employee.fullName);
        test_1.expect.soft(expectedVals[2].trim()).toEqual(employee.role);
        test_1.expect.soft(expectedVals[3].trim()).toEqual(employee.email);
        test_1.expect.soft(expectedVals[4].trim()).toEqual(employee.location);
        test_1.expect.soft(expectedVals[5].trim()).toEqual(employee.department);
    });
    async function datePicker(dateInput, page) {
    ...
    ;
    async function getSecondRow(firstPage, secondPage) {
        const listOfrows = [];
